...
# referrer field of a Common Log Format access log for a specified
# domain name.
...
for URL in $(awk '{ if (length($11) > 4) { print $11 } }' "$1" | grep $2)
do
    args="$(echo $URL | cut -d\? -f2 | tr '&' '\n' | grep -E '(^q=|^sid=|^p=|query=|item=|ask=|name=|topic=)' | cut -d= -f2)"
