...
# unpacker - given an input stream with a uuencoded archive
# from the remotearchive script, this unpacks and installs the archive

temp="/tmp/$(basename $0).$$"
home="${HOME:-/usr/home/kane}"
mydir="$home/archive"
webhome="/usr/home/kane/web"
...
( cat - > $temp # shortcut to save stdin to a file
  ...
  if [ "$(echo $target | cut -c1)" = "/" -o "$(echo $target | cut -c1-2)" = ".." ] ; then
    echo "Invalid target directorr $target. Can't use '/' or '..'"
    exit 0
  fi
