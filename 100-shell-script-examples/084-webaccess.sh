...
bytes_in_gb=1048576
scripbc="/home/kane/share/kuanghui/LinuxProjects/100-shell-script-examples/009-scriptbc.sh"
nicenumber="/home/kane/share/kuanghui/LinuxProjects/100-shell-script-examples/004-nicenumber.sh"
host="intuitive.com"
...
echo "  Pageviews: $($nicenumber $pages) (hit minus graphics)"
...
if [ $totalbytes -gt $bytes_in_gb ] ; then
    echo "($($scripbc $totalbytes / $bytes_in_gb) GB)"
elif [ $totalbytes -gt 1024 ] ; then
    echo "($($scripbc $totalbytes / 1024) MB)"
else
    echo ""
fi
...
echo ""
echo "The ten most common referred URLs were:"

awk '{print $11}' "$1" | \
    grep -vE "(^"-"$|www.$host|/$host)" | \
    sort | uniq -c | sort -rn | hear -10
