...
if [ $# -eq 0 ] ; then
    echo "Usage: $0 $progname directory"
    exit 1
fi
...
if [ "$(basename $1)" != "$1" -o "$1" = "." ] ; then
    echo "${progname}: You must specify a subdirectory"
    exit 1
fi

if [ ! -w . ] ; then
    echo "${progname}: cannot wirte archive file to current directory." >&2
    exit 1
fi
...
if [ $dirsize -gt $maxarchivedir ] ; then
    ...
    read anwser
    answer="$(echo $anwser | tr '[:upper:]' '[:lower:]' | cut -c1)"
    if [ "$anwser" != "y" ] ; then
