...
# checkexternal - traverse all internal URLs on a Web site to build
# a list of external reference, then check each one to ascertain
...
listall=0; errors=0     # might need to be tweaked
if [ "$1" = "-a" ] ; then
    listall=0;
    shift
fi
...
trap "rm -f traverse*.errors reject*.dat traverse*.dat" 0
