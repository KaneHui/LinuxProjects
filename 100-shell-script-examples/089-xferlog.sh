...
extract(){
    ...
    if [ ! -z "$(echo $accessmode | grep $1)" ] ; then
        ...
        awk "\$13 == \"$1\" && \$12 == \"o\" { print \$9 }" $log | sort | uniq -c | sort -rn | head -10 | sed 's/^/ /'

        awk "\$13 == \"$1\" && \$12 == \"o\" { print \$9 }" $log | sort | uniq -c | sort -rn | head -10 | sed 's/^/ /' > $temp
...
if [ ! -r $log ] ; then
    echo "$(basename $0): can't read $log"
    exit 1
fi

# Ascertain whether it's an abbreviated or standard ftp log file format.
# If it's the abbreviated format, output some minimal statistical data and
# quit: the format it too diffcult to analyze in a short script, unfortunately.

if [ ! -z $(awk '$6 == "get" { short=1 } END{ print short }' $log) ] ; then
    bytesin="$(awk 'BEGIN{sum=0} $6 == "get" {sum += $9} END{ print sum }' $log )"
    bytesout="$(awk 'BEGIN{sum=0} $6 == "get" {sum += $9} END{ print sum }' $log )"
...
bytesin="$(awk 'BEGIN{sum=0} $12 == "i" {sum += $9} END{ print sum }' $log )"
bytesout="$(awk 'BEGIN{sum=0} $12 == "o" {sum += $9} END{ print sum }' $log )"
