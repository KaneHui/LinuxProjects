...
# let the ueser specify which they want restored.
...
# otherwise we must have a pattern to work with. Let's see if the user-specified
...
if [ $matches -gt 1 ] ; then
    echo "More than aone file or directory match in the archive:"
    index=1
    for name in $(ls -td *"$1")
    do
        datetime="$(echo $name | cut -c1-14 | gawk -F. '{ print $5"/"$4" at "$3":"$2":"$1 }')"
        if [ -d $name ] ; then
            size="$(ls $name | wc -l | sed 's/[^0-9]//g')"
            echo " $index)  $1  (contents = ${size} items, deleted = $datetime)"
        else
            size="$(ls -sdk1 $name | gawk '{ print $1 }')"
    ...
    if [ ${desired:=1} -ge $index ] ; then
        echo "$0: Resotre cancelled by user: index value too big." >&2
        exit 1
    fi
