...
# and simultaneously step through each home directory to find those that
# contain a .slocatedb file. If found, an additional, private version of
# the locate database will be created for that user.
...
if [ "$(grep '^nobody:' /etc/passwd)" = "" ] ; then
    echo "$0: Error: you must have an account for user 'nody'" >&2
...
# has a $slocatedb file in their home directory.....
...
do
    ...
    if [ "$homedir" = "/" ] ; then
        ...
        su -fm $account -c "find / -print" > $homedir/$slocatedb 2>/dev/null
        chmod 600 $homedir/slo
        chmod $account $homedir/$slocatedb
