...
# formatted properly. Expects standard cron notation of min hr dom
# mon dow CMD
...
if [ $# -ne 1 ] || [ ! -r $1 ] ; then
    echo "Usage: $0 usercrontablefile" >&2
    exit 1
fi
...
do
    ...
    if [ -z "$min" -o "${min:0:1}" = "#" ] ; then
        continue
    elif [ ! -z $(echo ${min:0:1} | sed 's/[[:digit:]]//g') ] ; then
